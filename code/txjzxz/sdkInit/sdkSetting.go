...
func CreateCCLifecycle(info *SdkEnvInfo, sequence int64, upgrade bool, sdk *fabsdk.FabricSDK) error {
	...
	// Package cc
	fmt.Println(">> 开始打包链码......")
	label, ccPkg, err := packageCC(info.ChaincodeID1, info.ChaincodeVersion1, info.ChaincodePath)
	if err != nil {
	...
	if err := approveCC(packageID, info.ChaincodeID1, info.ChaincodeVersion1, sequence, info.ChannelID, info.Orgs, info.OrdererEndpoint); err != nil {
		return fmt.Errorf("approveCC error: %v", err)
	}

	// Query approve cc
	if err:=queryApprovedCC(info.ChaincodeID1, sequence, info.ChannelID, info.Orgs);err!=nil{
	...
	if err:=checkCCCommitReadiness(packageID, info.ChaincodeID1, info.ChaincodeVersion1, sequence, info.ChannelID, info.Orgs); err!=nil{
	...
	if err:=commitCC(info.ChaincodeID1, info.ChaincodeVersion1, sequence, info.ChannelID, info.Orgs, info.OrdererEndpoint);err!=nil{
		return fmt.Errorf("commitCC error: %v", err)
	}
	// Query committed cc
	if err:=queryCommittedCC(info.ChaincodeID1, info.ChannelID, sequence, info.Orgs); err!=nil{
	...
	if err:=initCC(info.ChaincodeID1, upgrade, info.ChannelID, info.Orgs[0], sdk); err!=nil{
...
func installCC(label string, ccPkg []byte, orgs []*OrgInfo) error {
	...
	for _, org := range orgs {
		...
		if flag, _ := checkInstalled(packageID, orgPeers[0], org.OrgResMgmt); flag == false {
			if p, err := org.OrgResMgmt.LifecycleInstallCC(installCCReq, resmgmt.WithTargets(orgPeers...), resmgmt.WithRetry(retry.DefaultResMgmtOpts)); err != nil {
...
func approveCC(packageID string, ccName, ccVersion string, sequence int64, channelID string, orgs []*OrgInfo, ordererEndpoint string) error {
	...
	// org1OrOrg2 := "AND('Org1MSP.member','Org2MSP.member','Org3MSP.member','Org6MSP.member')"
